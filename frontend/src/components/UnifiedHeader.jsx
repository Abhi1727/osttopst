...
const UnifiedHeader = ({ session, onReset }) => {
  ...
  const megaMenuProducts = [
    ...
    },
    {
      ...
      icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M22 6l-10 7L2 6"/><line x1="12" y1="13" x2="12" y2="13"/></>,
    },
  ];
  ...
  const supportedPlatforms = [
    ...
    },
    {
      ...
      icon: <><rect x="2" y="2" width="20" height="8" rx="2" ry="2"/><rect x="2" y="14" width="20" height="8" rx="2" ry="2"/><line x1="6" y1="6" x2="6" y2="6"/><line x1="6" y1="18" x2="6" y2="18"/></>,
    },
    {
      label: "Office 365",
    ...
    {
      label: "Gmail",
      icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M22 7l-10 7L2 7"/></>,
    },
    {
      label: "Yahoo Mail",
      icon: <><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M2 6l10 7 10-7"/></>,
    },
    {
      label: "All OST Files",
      icon: <><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/><path d="M3 12c0 1.66 4 3 9 3s9-1.34 9-3"/></>,
                ...
                >
                  <div className="bg-white rounded-2xl shadow-[0_20px_60px_-12px_rgba(0,0,0,0.22)] border border-slate-100 overflow-hidden w-full max-w-[1100px] flex">
                    ...
                    <div className="w-[260px] shrink-0 border-r border-slate-100 bg-slate-50/60 p-4 flex flex-col gap-1">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.18em] mb-2 px-3">Categories</p>
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-[14px] font-bold transition-colors ${
                            activeCategory === "converter" ? "text-brand-600" : "text-slate-700 group-hover:text-brand-600"
                          }`}>OST Converter</span>
                          <ChevronRight size={15} className={`transition-all ${
                            activeCategory === "converter" ? "text-brand-400" : "text-slate-300 group-hover:text-brand-400"
                          }`} />
                        </div>
                        <p className="text-[12px] text-slate-500 leading-snug">Convert OST to PST, PDF, JSON, MBOX, EML &amp; MSG</p>
                      </button>
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-[14px] font-bold transition-colors ${
                            activeCategory === "viewer" ? "text-brand-600" : "text-slate-700 group-hover:text-brand-600"
                          }`}>OST Viewer</span>
                          <ChevronRight size={15} className={`transition-all ${
                            activeCategory === "viewer" ? "text-brand-400" : "text-slate-300 group-hover:text-brand-400"
                          }`} />
                        </div>
                        <p className="text-[12px] text-slate-500 leading-snug">Read OST files online without Microsoft Outlook</p>
                      </button>
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className={`text-[14px] font-bold transition-colors ${
                            activeCategory === "others" ? "text-brand-600" : "text-slate-700 group-hover:text-brand-600"
                          }`}>Other Tools</span>
                          <ChevronRight size={15} className={`transition-all ${
                            activeCategory === "others" ? "text-brand-400" : "text-slate-300 group-hover:text-brand-400"
                          }`} />
                        </div>
                        <p className="text-[12px] text-slate-500 leading-snug">Additional tools for OST file management</p>
                      </button>
                    ...
                    <div className="flex-1 p-6">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.18em] mb-4 px-1">Products</p>
                        ...
                        {(activeCategory === "converter"
                          ...
                          <button
                            ...
                            className={`flex items-start gap-4 px-4 py-4 rounded-xl transition-all text-left group ${
                              prod.path ? "hover:bg-slate-50 hover:shadow-sm cursor-pointer" : "cursor-default opacity-60"
                            }`}
                          >
                            <div className="w-11 h-11 rounded-xl bg-brand-50 flex items-center justify-center shrink-0 group-hover:bg-brand-100 transition-colors">
                              <svg viewBox="0 0 24 24" fill="none" stroke="#0ea5e9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                                {prod.icon}
                              </svg>
                            </div>
                            <div className="min-w-0">
                              <p className="text-[14px] font-bold text-slate-800 group-hover:text-brand-600 transition-colors leading-tight">{prod.label}</p>
                              <p className="text-[12px] text-slate-400 mt-1 leading-snug line-clamp-2">{prod.desc}</p>
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="w-[220px] shrink-0 border-l border-slate-100 bg-slate-50/40 p-5">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.18em] mb-4">What We Support</p>
                      <div className="flex flex-col gap-0.5">
                        {supportedPlatforms.map((platform) => (
                          <div key={platform.label} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white hover:shadow-sm transition-all cursor-default">
                            <svg viewBox="0 0 24 24" fill="none" stroke="#0ea5e9" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 shrink-0">
                              {platform.icon}
                            </svg>
                            <span className="text-[13px] text-slate-700 font-medium">{platform.label}</span>
                          </div>
                        ))}
                      </div>
                    </div>
